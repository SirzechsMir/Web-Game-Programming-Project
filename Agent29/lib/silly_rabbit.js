...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const manifest = [
    ...
    {src: "C:\Users\300924829\Desktop\SillyRabbit-master\assets\images", id: "bg"},
	...
	{src: "images/cereal.png", id: "power"},
...
const start = () => {
  ...
  for(let i = 0; i < 3; i++){
    ...
    const powerImg = loader.getResult("power");
  const y = height - powerImg.height - groundImg.height + 50;
  for(let i = 0; i < 3; i++){
    let x = Math.random() * width + (width * (i + 1))/3 + width;
    powerup = new Powerup(stage, powerImg, x, y);
    Powers.push(power);
  }
